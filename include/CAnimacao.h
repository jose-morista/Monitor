...
CAnimacao(int px,int py,char *nomeArq,int usaGerenciadorTimer=0,int retiraFundo=1,int opacidade=255,int idJanela=0):CObjeto(px,py,nomeArq,retiraFundo,opacidade,idJanela){
    souCopia = 0;
    offx = offy = 0;
    modoAtual = 0;
    frameDoModo=-1;
...
CAnimacao(int px,int py,CAnimacao* base,int usaGerenciadorTimer=0,int idJanela=0):CObjeto(px,py,base->nomeImagem,idJanela){
    souCopia= 1;
    frameDoModo=-1;
...
~CAnimacao(){
    ...
    if (tempoFrame)
        delete tempoFrame;
    else CGerenciadorTimers::DestroiTimer(idTimer);
}
    ...
    if (tempoDecorrido>tempo){
        ...
        if (frameDoModo>=modo->qtdFrames){//testa se chegou ao ultimo frame do modo atual
            if (modo->loop){
                ...
                frameDoModo--; //permanece no ultim frame e informa o encerramento do modo com resp=1
                resp=1;
            }
        }
        if (tempoFrame)
