...
public:
    ...
    CJogo(char *nome,int cursor=0,int altura=ALT_TELA,int largura=LARG_TELA){
        ...
        fontes[0] = new CMapaCaracteres("..//fontes//arial.ttf",CGerenciadorJanelas::GetJanela(0)->GetRenderer(),36,ESTILO_NORMAL,BRANCO,0,BRANCO);
        fontesUsadas = 1;
    }
    ...
    PIG_Evento PegaEvento(){
        ...
        if (SDL_PollEvent((&event))){
            ...
            case SDL_MOUSEBUTTONDOWN:
                ...
                ultimoEvento.mouse.numeroJanela = event.window.windowID-1;
            ...
            case SDL_WINDOWEVENT:
                if (event.window.event==JANELA_FECHADA){
                    CGerenciadorJanelas::GetJanela(event.window.windowID-1)->Fecha();
                }
                ultimoEvento.tipoEvento = EVENTO_JANELA;
                ultimoEvento.janela.acao = event.window.event;
                ultimoEvento.janela.numeroJanela = event.window.windowID-1;
    ...
    void DesenhaLinhaSimples(int x1,int y1,int x2,int y2,PIG_Cor cor,int idJanela=0){
        ...
        SDL_RenderDrawLine(CGerenciadorJanelas::GetJanela(idJanela)->GetRenderer(),x1,ALT_TELA-y1,x2,ALT_TELA-y2);
    }
    ...
    int CriaFonteFundo(char *nome,int tamanho,int estilo,char *arquivoFundo,int contorno,PIG_Cor corContorno,int idJanela=0){
        ...
        fontes[fontesUsadas] = new CMapaCaracteres(nome,CGerenciadorJanelas::GetJanela(idJanela)->GetRenderer(),tamanho,estilo,arquivoFundo,contorno,corContorno);
        fontesUsadas++;
        return aux;
    }
    ...
    int CriaFonteNormal(char *nome,int tamanho,int estilo,PIG_Cor corLetra,int contorno,PIG_Cor corContorno,int idJanela=0){
        int aux = fontesUsadas;
        fontes[fontesUsadas] = new CMapaCaracteres(nome,CGerenciadorJanelas::GetJanela(idJanela)->GetRenderer(),tamanho,estilo,corLetra,contorno,corContorno);
        fontesUsadas++;
        return aux;
    }
